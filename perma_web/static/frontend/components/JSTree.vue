...
function folderListFromUrl() {
  ...
  let queryDict = new URLSearchParams(window.location.search);
  if (queryDict.get('folder')) {
    try {
      let folder_list = queryDict.get('folder').split('-');
...
/* end move to foldertree */


function getPathForId(folderId) {
...
<template>
  <div ref="folderTreeRef" id="folder-tree"></div>
</template>
